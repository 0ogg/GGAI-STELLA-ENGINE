...
### 시나리오 — 폴더
`GGAI/SCENARIOS/<이름>/scenario.json` + `SESSIONS/` + 선택적 `assets/`. CCv3 스펙 그대로, 플러그인 메타는 `data.extensions.stella`({id, favorite, lastPlayedAt, playCount, thumbnail, translationLorebookIds?, illustrationLorebookIds?, …})에 격리. 이름 중복 허용(id 로 구분). 시나리오 루트 `assets/`는 모든 세션이 공유하는 ST 호환 캐릭터 에셋이다. 타입: `src/types/scenario.ts`.
...
### 세션 — 폴더 + session.json
`GGAI/SCENARIOS/<시나리오>/SESSIONS/<세션>/session.json` + `assets/` + 미디어 JSON(translations/illustrations/summaries). 세션 `assets/`는 채팅 첨부·생성 미디어다.
...
- 노드 kind: `root` | `ai-continue` | `ai-regen` | `user-write` | `user-edit`. AI 노드는 `gen` 메타. `proactive:true` = 선채팅 발화. 챗의 사용자 노드는 선택적으로 `attachments[]`(`kind:image`, 세션 상대 `path`, MIME, 캡션)을 가지며 실제 파일은 같은 세션의 `assets/`에 둔다.

...
export interface SummaryActiveSettings {
   ...
   * 한 덩어리로 압축한다(컴팩트). 0/미설정이면 토큰 기준 압축을 하지 않는다.
   */
  maxTokens?: number;
  /**
   * 원본 그대로 유지할 최근 사건 블록 수 — 그 앞은 자동으로 한 덩어리로 접는다.
   * 생략 시 4, 0 이면 개수 기준 압축을 하지 않는다.
   */
  keepRecent?: number;
  /**
   * 나눠 배치 — 「지난 이야기」는 본문 앞(로어북 뒤)에, 「현재 상황」만 본문 끝
